 #!/usr/bin/env python3
  ...
  Tracks email opens via pixel tracking and link clicks
  """
  ...
  # 1x1 transparent pixel (hardcoded bytes - no PIL needed)
  TRACKING_PIXEL = b'\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\x80\x00\x00\x00\x00\x00\x00\x00\x00\x21\xf9\x04\x01\x00\x00\x00\x00\x2c\x00\x
  00\x00\x00\x01\x00\x01\x00\x00\x02\x02\x44\x01\x00\x3b'
  ...
  def view_link_stats():
      ...
      html += '<style>table {border-collapse: collapse; width: 100%;} th, td {border: 1px solid #ddd; padding: 8px; text-align: left;} th
  {background-color: #f2f2f2;}</style>'
      ...
      if not results:
          ...
          for row in results:
              ...
              html += f'<tr><td>{row[0]}</td><td
  title="{row[1]}">{url_display}</td><td>{row[2]}</td><td>{last_clicked}</td><td>{row[4]}</td></tr>'
  ...
  def view_stats():
      ...
      html += '<style>table {border-collapse: collapse; width: 100%;} th, td {border: 1px solid #ddd; padding: 8px; text-align: left;} th
  {background-color: #f2f2f2;}</style>'
      html += '</head><body><h1>📊 Email Open Statistics</h1>'
      ...
      if not results:
          html += '<tr><td colspan="6">No emails tracked yet.</td></tr>'
      else:
          for row in results:
              opened = row[3] if row[3] else 'Not opened'
              opens = row[4] if row[4] else 0
              html += f'<tr><td>{row[0]}</td><td>{row[1]}</td><td>{row[2]}</td><td>{opened}</td><td>{opens}</td><td>{row[5] or
  "N/A"}</td></tr>'
